...
/*!
	...
	\return relationship as a \link SQLITE_CIRCLE_PATH_REL long value \endlink (\ref SQLITE_CIRCLE_PATH_REL )
*/
static long path_circle_pos ( const SPath * path, const SCircle * circ ) {
	...
	static SLine sl;
	static long n;
	static const long sc_in = ( 1 << SQLITE_CIRCLE_CONT_LINE );
	static const long sc_ov = ( 1 << SQLITE_CIRCLE_LINE_OVER );
	static const long sc_os = ( 1 << SQLITE_CIRCLE_LINE_AVOID );
	...
	if ( FPzero( circ->radius ) ) {
		...
		if ( pcp ) {
			return SQLITE_CIRCLE_PATH_OVER;
		} else {
			return SQLITE_CIRCLE_PATH_AVOID;
		}
	}
	...
	if ( pos == sc_in ) {                   // all lines inside  circle
		...
		return SQLITE_CIRCLE_CONT_PATH; 
...
static bool path_line_overlap ( const SPath * path, const SLine * line ) {
	...
	for ( i=0 ; i<n; i++ ) {
		spath_segment ( &sl, path, i );
		if ( sline_sline_pos ( &sl, line  ) != SQLITE_LINE_AVOID ) {
			return true;
		}
	}
	return false;
...
/*!
	...
	\return relationship as a \link SQLITE_ELLIPSE_PATH_REL long value \endlink (\ref SQLITE_ELLIPSE_PATH_REL )
*/
static long  path_ellipse_pos ( const SPath * path, const SEllipse * ell ) {
	...
	static SLine sl;
	static long n;
	static const long sc_in = ( 1 << SQLITE_ELLIPSE_CONT_LINE );
	static const long sc_ov = ( 1 << SQLITE_ELLIPSE_LINE_OVER );
	static const long sc_os = ( 1 << SQLITE_ELLIPSE_LINE_AVOID );
	...
	if ( FPzero( ell->rad[0] ) ) {
		...
		if ( pcp ) {
			return SQLITE_ELLIPSE_PATH_OVER;
		} else {
			return SQLITE_ELLIPSE_PATH_AVOID;
		}
	}
	...
	if ( pos == sc_in ) {                   // all lines inside  ellipse
		...
		return SQLITE_ELLIPSE_CONT_PATH; 
...
/*!
	...
	\return relationship as a \link SQLITE_POLY_PATH_REL long value \endlink (\ref SQLITE_POLY_PATH_REL )
*/
static long  path_poly_pos ( const SPath * path, const SPoly * poly ) {
	...
	static long pos, res;
	static long n;
	static const long sp_os = ( 1 << SQLITE_LINE_POLY_AVOID );
	static const long sp_ct = ( 1 << SQLITE_POLY_CONT_LINE );
	static const long sp_ov = ( 1 << SQLITE_LINE_POLY_OVER );
	...
	for ( i=0 ; i<n; i++ ) {
		...
		if ( pos == sp_ov ) {
			return SQLITE_POLY_PATH_OVER; // overlap
		}
		res |= pos;
	}

	if ( res == sp_os ) {
		return SQLITE_POLY_PATH_AVOID;
	}
	if ( res == sp_ct ) {
		return SQLITE_POLY_CONT_PATH;
	}
...
bool spherecircle_cont_path (SCircle * circ, SPath * path) {
	return ( path_circle_pos ( path, circ ) == SQLITE_CIRCLE_CONT_PATH );
}
...
bool spherecircle_overlap_path (SCircle * circ, SPath * path) {
	return ( path_circle_pos ( path, circ ) > SQLITE_CIRCLE_PATH_AVOID );
}
...
bool spherepoly_cont_path (SPoly * poly, SPath * path) {
	return ( path_poly_pos ( path, poly ) == SQLITE_POLY_CONT_PATH );
}
...
bool spherepoly_overlap_path (SPoly * poly, SPath * path) {
	return ( path_poly_pos ( path, poly ) != SQLITE_POLY_PATH_AVOID );
}
...
bool  sphereellipse_cont_path (SEllipse * ell, SPath * path) {
	return ( path_ellipse_pos ( path, ell ) == SQLITE_ELLIPSE_CONT_PATH );
}
...
bool sphereellipse_overlap_path (SEllipse * ell, SPath * path) {
	return ( path_ellipse_pos ( path, ell ) != SQLITE_ELLIPSE_PATH_AVOID );
}

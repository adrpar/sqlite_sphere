...
static bool spherepoly_check ( const SPoly * poly ) {
	...
	for ( i=0; i < poly->npts; i++ ) {
		...
		for ( k=(i+1); k < poly->npts; k++ ) {
			...
			if ( ! ( pos == SQLITE_LINE_CONNECT || pos == SQLITE_LINE_AVOID ) ) {
				return false;
			}
		}
	}
...
/*!
	...
	\param poly pointer to polygon
	\return relationship as a \link SQLITE_CIRCLE_POLY_REL long long value \endlink (\ref SQLITE_CIRCLE_POLY_REL )
*/
static long long  poly_circle_pos ( const SPoly * poly, const SCircle * circ ) {
	...
	SLine sl;
	bool pcp;
	const long long sc_in = ( 1 << SQLITE_CIRCLE_CONT_LINE );
	const long long sc_ov = ( 1 << SQLITE_CIRCLE_LINE_OVER );
	const long long sc_os = ( 1 << SQLITE_CIRCLE_LINE_AVOID );

	pcp = spoly_contains_point ( poly, &circ->center );

	if ( FPzero( circ->radius ) ) {
		if ( pcp ) {
			return SQLITE_POLY_CONT_CIRCLE;
		} else {
			return SQLITE_CIRCLE_POLY_AVOID;
		}
	}
	...
	for ( i=0; i < poly->npts; i++ ) {
		...
		if ( ( ( pos & sc_in ) && ( pos & sc_os ) ) || ( pos & sc_ov ) ) {
			return SQLITE_CIRCLE_POLY_OVER; // overlaps
		}
	}

	if ( ( pos & sc_in ) == pos ) {         // all lines inside  circle
		// circle contains polygon
		return SQLITE_CIRCLE_CONT_POLY; 
	...
	return SQLITE_CIRCLE_POLY_OVER;
}
...
/*!
	...
	\param poly pointer to polygon
	\return relationship as a \link SQLITE_ELLIPSE_POLY_REL long long value \endlink (\ref SQLITE_ELLIPSE_POLY_REL )
*/
static long long  poly_ellipse_pos ( const SPoly * poly, const SEllipse * ell ) {
	...
	bool pcp;
	SPoint cen;
	const long long sc_in = ( 1 << SQLITE_ELLIPSE_CONT_LINE );
	const long long sc_ov = ( 1 << SQLITE_ELLIPSE_LINE_OVER );
	const long long sc_os = ( 1 << SQLITE_ELLIPSE_LINE_AVOID );
	...
	if ( FPzero( ell->rad[0] ) ) {
		if ( pcp ) {
			return SQLITE_POLY_CONT_ELLIPSE;
		} else {
			return SQLITE_ELLIPSE_POLY_AVOID;
		}
	}
	...
	if ( FPzero( ell->rad[1] ) ) {
		...
		if ( res == SQLITE_LINE_POLY_AVOID ) {
			return SQLITE_ELLIPSE_POLY_AVOID;
		} else if ( res == SQLITE_POLY_CONT_LINE ) {
			return SQLITE_POLY_CONT_ELLIPSE;
		} else {
			return SQLITE_ELLIPSE_POLY_OVER;
		}
	}
	...
	for ( i=0; i < poly->npts; i++ ) {
		...
		if ( ( ( pos & sc_in ) && ( pos & sc_os ) ) || ( pos & sc_ov ) ) {
			return SQLITE_ELLIPSE_POLY_OVER; // overlaps
		}
	}

	if ( ( pos & sc_in ) == pos ) {         // all lines inside  ellipse
		// ellipse contains polygon
		return SQLITE_ELLIPSE_CONT_POLY; 
...
/*!
	...
	\return relationship as a \link SQLITE_POLY_REL long long value \endlink (\ref SQLITE_POLY_REL )
*/
static long long  poly_poly_pos ( const SPoly * p1, const SPoly * p2 , bool recheck) {
	...
	const static long long sp_os = ( 1 << SQLITE_LINE_POLY_AVOID );
	const static long long sp_ct = ( 1 << SQLITE_POLY_CONT_LINE );
	const static long long sp_ov = ( 1 << SQLITE_LINE_POLY_OVER );

	for ( i=0; i < p2->npts; i++ ) {
		spoly_segment ( &sl, p2, i );
		pos = ( 1 << poly_line_pos ( p1, &sl ) );
		if ( pos == sp_ov ) {
			return SQLITE_POLY_OVER; // overlap
		}
		res |= pos;
	}
	...
	if ( res == sp_os ) {
		...
		if ( pos == SQLITE_POLY_CONT ) {
			return SQLITE_POLY_OVER;
		} else {
			return SQLITE_POLY_AVOID;
		}
	}
	if ( res == sp_ct ) {
		return SQLITE_POLY_CONT;
	}
	return SQLITE_POLY_OVER;
}  
		...
		*/
		...
		do {
			...
			if ( cntr > 10000 ) {
				...
				fprintf(stderr, "ERROR: Please report it to SQLITE_sphere team!");
				return false;
			}
			cntr++;
		} while ( eqa );
...
/*!
	...
	\param poly pointer to polygon
	\return relationship as a \link SQLITE_LINE_POLY_REL long long value \endlink (\ref SQLITE_LINE_POLY_REL )
*/
long long poly_line_pos ( const SPoly * poly, const SLine * line ) {
	...
	const static long long sl_os = ( 1 << SQLITE_LINE_AVOID );
	const static long long sl_cl = ( 1 << SQLITE_LINE_CONT_LINE );
	const static long long sl_cr = ( 1 << SQLITE_LINE_CROSS );
	const static long long sl_cn = ( 1 << SQLITE_LINE_CONNECT );
	const static long long sl_ov = ( 1 << SQLITE_LINE_OVER );
	const static long long sl_eq = ( 1 << SQLITE_LINE_EQUAL );
	...
	for ( i=0 ; i < poly->npts; i++ ) {
		...
		if ( pos == sl_ov ) {
			return SQLITE_LINE_POLY_OVER; // overlap
		}
	...
	if ( ( res & sl_cl ) && ( ( res - sl_cl - sl_os - sl_cn - 1 ) < 0 ) ) {
		return SQLITE_POLY_CONT_LINE;
	} else if (  p1 &&  p2 && ( ( res - sl_os - sl_cn - 1 ) < 0 ) ) {
		return SQLITE_POLY_CONT_LINE;
	} else if ( !p1 && !p2 && ( ( res - sl_os - 1 ) < 0 ) ) {
		return SQLITE_LINE_POLY_AVOID;
	}
	return SQLITE_LINE_POLY_OVER;
}  
...
bool spherepoly_cont_circle (SPoly * poly, SCircle * circ) {
	return ( poly_circle_pos ( poly, circ ) == SQLITE_POLY_CONT_CIRCLE );
}
...
bool spherecircle_cont_poly (SCircle * circ, SPoly * poly) {
	return ( poly_circle_pos ( poly, circ ) == SQLITE_CIRCLE_CONT_POLY );
}
...
bool spherepoly_overlap_circle (SPoly * poly, SCircle * circ) {
	return ( poly_circle_pos ( poly, circ ) > SQLITE_CIRCLE_POLY_AVOID );
}
...
bool spherepoly_cont_line (SPoly * poly, SLine * line) {
	return ( poly_line_pos ( poly, line ) == SQLITE_POLY_CONT_LINE );
}
...
bool spherepoly_overlap_line (SPoly * poly, SLine * line) {
	return ( poly_line_pos ( poly, line ) > SQLITE_LINE_POLY_AVOID );
}
...
bool spherepoly_cont_poly (SPoly * poly1, SPoly * poly2) {
	return ( poly_poly_pos ( poly1, poly2, false ) ==  SQLITE_POLY_CONT );
}
...
bool spherepoly_overlap_poly (SPoly * poly1, SPoly * poly2) {
	return ( poly_poly_pos ( poly1, poly2, false ) > SQLITE_POLY_AVOID );
}
...
bool spherepoly_cont_ellipse (SPoly * poly, SEllipse * ell) {
	return ( poly_ellipse_pos ( poly, ell ) == SQLITE_POLY_CONT_ELLIPSE );
}
...
bool sphereellipse_cont_poly (SEllipse * ell, SPoly * poly) {
	return ( poly_ellipse_pos ( poly, ell ) == SQLITE_ELLIPSE_CONT_POLY );
}
...
bool spherepoly_overlap_ellipse (SPoly * poly, SEllipse * ell) {
	return ( poly_ellipse_pos ( poly, ell ) > SQLITE_ELLIPSE_POLY_AVOID );
}

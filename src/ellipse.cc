...
/*!
	...
	\return relationship as a \link SQLITE_ELLIPSE_REL long long value \endlink (\ref SQLITE_ELLIPSE_REL )
*/
static long long sellipse_ellipse_pos ( const SEllipse * se1 , const SEllipse * se2 ) {
	...
	if ( FPeq ( se2->rad[0], se2->rad[1] ) ) {
		...
		switch ( r ) {
			case SQLITE_ELLIPSE_CIRCLE_AVOID: 
				return SQLITE_ELLIPSE_AVOID;
			case SQLITE_CIRCLE_CONT_ELLIPSE: 
				return SQLITE_ELLIPSE_OVER;
			case SQLITE_ELLIPSE_CONT_CIRCLE: 
				return SQLITE_ELLIPSE_CONT;
			case SQLITE_ELLIPSE_CIRCLE_OVER: 
				return SQLITE_ELLIPSE_OVER;
			default: 
				return SQLITE_ELLIPSE_AVOID;
		}
	...
	if ( FPzero ( se2->rad[1] ) ) {
		...
		switch ( r ) {
			case SQLITE_ELLIPSE_LINE_AVOID: 
				return SQLITE_ELLIPSE_AVOID;
			case SQLITE_ELLIPSE_CONT_LINE: 
				return SQLITE_ELLIPSE_CONT;
			case SQLITE_ELLIPSE_LINE_OVER: 
				return SQLITE_ELLIPSE_OVER;
			default: 
				return SQLITE_ELLIPSE_AVOID;
		}
...
long long sellipse_line_pos ( const SEllipse * se , const SLine * sl ) {
	...
	if ( FPzero(sl->length) ) {
		...
		if ( sellipse_cont_point ( se, sline_begin( &p, sl ) ) ) {
			return SQLITE_ELLIPSE_CONT_LINE;
		} else {
			return SQLITE_ELLIPSE_LINE_AVOID;
		}
	} 
	...
	if ( FPzero(se->rad[0]) ) {
		...
		if ( spoint_at_sline( &p, sl ) ) {
			return SQLITE_ELLIPSE_LINE_OVER;
		} else {
			return SQLITE_ELLIPSE_LINE_AVOID;
		}
	}
	...
	if ( FPzero(se->rad[1]) ) {
		...
		if (res == SQLITE_LINE_AVOID ) {
			return SQLITE_ELLIPSE_LINE_AVOID;
		} else if ( res == SQLITE_LINE_CONT_LINE || res == SQLITE_LINE_EQUAL ) {
			return SQLITE_ELLIPSE_CONT_LINE;
		} else {
			return SQLITE_ELLIPSE_LINE_OVER;
		}
	...
	if ( FPeq(se->rad[0], se->rad[1]) ) {
		...
		if ( res == SQLITE_CIRCLE_LINE_AVOID ) {
			return SQLITE_ELLIPSE_LINE_AVOID;
		} else if ( res == SQLITE_CIRCLE_CONT_LINE ) {
			return SQLITE_ELLIPSE_CONT_LINE;
		} else {
			return SQLITE_ELLIPSE_LINE_OVER;
		}
...
long long sellipse_circle_pos ( const SEllipse * se , const SCircle * sc ) {
	...
	if ( FPzero(sc->radius) ) {
		if ( sellipse_cont_point ( se , &sc->center ) ) {
			return SQLITE_ELLIPSE_CONT_CIRCLE;
		} else {
			return SQLITE_ELLIPSE_CIRCLE_AVOID;
		}
	}
	...
	if ( FPeq(se->rad[0],se->rad[1]) ) {
		...
		if ( scircle_eq ( &tc, sc ) ) {
			return SQLITE_ELLIPSE_CIRCLE_EQUAL;
		}
		...
		if ( FPle( ( dist + tc.radius ) , sc->radius ) ) {
			return SQLITE_CIRCLE_CONT_ELLIPSE;
		}
		if ( FPgt( ( sc->radius + tc.radius ) , dist ) ) {
			return SQLITE_ELLIPSE_CIRCLE_OVER;
		} else {
			return SQLITE_ELLIPSE_CIRCLE_AVOID;
		}
	}
...
bool sphereellipse_cont_line (SEllipse * e, SLine * l) {
	...
	return ( b == SQLITE_ELLIPSE_CONT_LINE );
}
...
bool sphereellipse_overlap_line (SEllipse * e, SLine * l) {
	...
	return ( b > SQLITE_ELLIPSE_LINE_AVOID );
}
...
bool sphereellipse_cont_circle (SEllipse * e, SCircle * c) {
	...
	return ( pos == SQLITE_ELLIPSE_CONT_CIRCLE || pos == SQLITE_ELLIPSE_CIRCLE_EQUAL );
}
...
bool spherecircle_cont_ellipse (SCircle * c, SEllipse * e) {
	int pos = sellipse_circle_pos ( e , c );
	return ( pos == SQLITE_CIRCLE_CONT_ELLIPSE || pos == SQLITE_ELLIPSE_CIRCLE_EQUAL );
}
...
bool sphereellipse_overlap_circle (SEllipse * e, SCircle * c) {
	return ( sellipse_circle_pos ( e , c ) > SQLITE_ELLIPSE_CIRCLE_AVOID );
}
...
bool sphereellipse_cont_ellipse (SEllipse * e1, SEllipse * e2) {
	return ( sellipse_ellipse_pos ( e1, e2 ) ==  SQLITE_ELLIPSE_CONT );
}
...
bool sphereellipse_overlap_ellipse (SEllipse * e1, SEllipse * e2) {
	return ( sellipse_ellipse_pos ( e1, e2 ) > SQLITE_ELLIPSE_AVOID );
}

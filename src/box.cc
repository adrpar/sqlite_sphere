...
\return relationship as a \link SQLITE_BOX_CIRCLE_REL long value \endlink (\ref  SQLITE_BOX_CIRCLE_REL )
*/
static long sbox_circle_pos ( const SCircle * sc , const SBox * sb ) {
	if ( FPlt( ( sc->center.lat + sc->radius ), sb->sw.lat ) ) {
		return SQLITE_BOX_CIRCLE_AVOID;
		...
		if ( spoint_eq ( &sb->ne, &tmpn ) && FPge( sc->center.lat - sc->radius, sb->sw.lat) ) {
			if ( spoint_eq( &sc->center, &tmpn ) && FPeq( sc->radius, ( PIH - sb->sw.lat ) ) ) {
				return SQLITE_BOX_CIRCLE_EQUAL;
			} else {
				return SQLITE_BOX_CONT_CIRCLE;
			}
		...
		if ( pw == SQLITE_CIRCLE_LINE_AVOID && pe == SQLITE_CIRCLE_LINE_AVOID ) {
			if ( bcc && lat_b_cont_c ) {
				return SQLITE_BOX_CONT_CIRCLE;
			}
			return SQLITE_BOX_CIRCLE_AVOID;
		}
		if ( pw == SQLITE_CIRCLE_CONT_LINE && pe == SQLITE_CIRCLE_CONT_LINE ) {
			if ( ccb ) {
				return SQLITE_CIRCLE_CONT_BOX;
			}
			return SQLITE_BOX_CIRCLE_OVER;
		}
		if ( bcc && lat_b_cont_c ) {
			bool touw = false , toue = false;
			if ( pw == SQLITE_CIRCLE_LINE_OVER ) {
			...
			if ( touw && toue ) {
				return SQLITE_BOX_CONT_CIRCLE;
			} else if ( touw && pe == SQLITE_CIRCLE_LINE_AVOID ) {
				return SQLITE_BOX_CONT_CIRCLE;
			}
			if ( toue && pw == SQLITE_CIRCLE_LINE_AVOID ) {
				return SQLITE_BOX_CONT_CIRCLE;
			}
			return SQLITE_BOX_CIRCLE_OVER;
		}
...
/*!
	...
	\param sb pointer to a box
	\return relationship as a \link SQLITE_BOX_LINE_REL long value \endlink (\ref  SQLITE_BOX_LINE_REL )
*/
static long sbox_line_pos ( const SLine * sl , const SBox * sb ) {
	...
	if ( FPzero( sl->length ) ) {
		if ( sbox_cont_point ( sb, &pbg ) ) {
			return SQLITE_BOX_CONT_LINE;
		} else {
			return SQLITE_BOX_LINE_AVOID;
		}
	}
	sline_min_max_lat ( sl, &minlat, &maxlat );

	if ( FPgt ( minlat, sb->ne.lat ) )  return SQLITE_BOX_LINE_AVOID;
	if ( FPlt ( maxlat, sb->sw.lat ) )  return SQLITE_BOX_LINE_AVOID; 
	...
	if ( pw == SQLITE_LINE_EQUAL || pe == SQLITE_LINE_EQUAL ) {
		return SQLITE_BOX_CONT_LINE;
	}
	if ( pw == SQLITE_LINE_CONT_LINE || pe == SQLITE_LINE_CONT_LINE ) {
		return SQLITE_BOX_CONT_LINE;
	}
	if ( pw == SQLITE_LINE_CROSS || pe == SQLITE_LINE_CROSS ) {
		if ( pw == SQLITE_LINE_CROSS ) {
			if ( !spoint_at_sline ( &pbg, &bw ) && !spoint_at_sline ( &ped, &bw ) ) {
				return SQLITE_BOX_LINE_OVER;
			}
		}
		if ( pe == SQLITE_LINE_CROSS ) {
			if ( !spoint_at_sline ( &pbg, &be ) && !spoint_at_sline ( &ped, &be ) ) {
				return SQLITE_BOX_LINE_OVER;
			}
		}
	}
	if ( pw == SQLITE_LINE_OVER || pe == SQLITE_LINE_OVER ) {
		return SQLITE_BOX_LINE_OVER;
	}
...
/*!
	...
	\return relationship as a \link SQLITE_BOX_PATH_REL long value \endlink (\ref SQLITE_BOX_PATH_REL )
*/
static long sbox_path_pos ( const SPath * path , const SBox * box ) {
	...
	static SLine sl;
	static long n;
	static const long sb_in = ( 1 << SQLITE_BOX_CONT_LINE );
	static const long sb_ov = ( 1 << SQLITE_BOX_LINE_OVER );
	static const long sb_os = ( 1 << SQLITE_BOX_LINE_AVOID );
	...
	if ( spoint_eq( &box->sw, &box->ne ) ) {
		if ( spath_cont_point ( path, &box->sw ) ) {
			return SQLITE_BOX_PATH_OVER;
		} else {
			return SQLITE_BOX_PATH_AVOID;
		}
	}
...
/*!
	...
	\param box  pointer to box
	\return relationship as a \link SQLITE_BOX_POLY_REL long value \endlink (\ref SQLITE_BOX_POLY_REL )
*/
static long sbox_poly_pos ( const SPoly * poly, const SBox * box ) {
	...
	SPoint sp;
	bool pcp;
	const long sb_in = ( 1 << SQLITE_BOX_CONT_LINE );
	const long sb_ov = ( 1 << SQLITE_BOX_LINE_OVER );
	const long sb_os = ( 1 << SQLITE_BOX_LINE_AVOID );
	...
	if ( spoint_eq( &box->sw, &box->ne) ) {
		if ( pcp ) {
			return SQLITE_POLY_CONT_BOX;
		} else {
			return SQLITE_BOX_POLY_AVOID;
		}
	}
	...
	for ( i=0; i < poly->npts; i++ ) {
		...
		if ( ( ( pos & sb_in ) && ( pos & sb_os ) ) || ( pos & sb_ov ) ) {
			return SQLITE_BOX_POLY_OVER; // overlaps
		}
	}

	if ( ( pos & sb_in ) == pos ) {         // all lines inside box
		// box contains polygon
		return SQLITE_BOX_CONT_POLY;
...
/*!
	...
	\return relationship as a \link SQLITE_BOX_ELLIPSE_REL long value \endlink (\ref  SQLITE_BOX_ELLIPSE_REL )
*/
static long sbox_ellipse_pos ( const SEllipse * ell , const SBox * box ) {
	...
	if ( spoint_eq( &box->sw, &box->ne) ) {
		...
		if ( scp ) {
			return SQLITE_ELLIPSE_CONT_BOX;
		} else {
			return SQLITE_BOX_ELLIPSE_AVOID;
		}
	}
	...
	// check by latitude
	if ( pi == SQLITE_CIRCLE_CONT_ELLIPSE || pi == SQLITE_ELLIPSE_CIRCLE_EQUAL ) {
		return SQLITE_BOX_ELLIPSE_AVOID;
	}
	if ( FPeq(sco.center.lat, sci.center.lat) ) {
		if ( po == SQLITE_ELLIPSE_CIRCLE_AVOID ) {
			return SQLITE_BOX_ELLIPSE_AVOID;
		}
		if ( FPzero(box->sw.lng) && FPeq(box->ne.lng, PID) ) {
			// full latitude range
			if ( ( po == SQLITE_CIRCLE_CONT_ELLIPSE || po == SQLITE_ELLIPSE_CIRCLE_EQUAL ) && 
					pi == SQLITE_ELLIPSE_CIRCLE_AVOID ) {
				return SQLITE_BOX_CONT_ELLIPSE;
			} else {
				return SQLITE_BOX_ELLIPSE_OVER;
			}
		}
	} else {
		// equator modus
		...
		if ( FPzero(box->sw.lng) && FPeq(box->ne.lng, PID) ) {
			// full latitude range
			if ( po == SQLITE_ELLIPSE_CIRCLE_AVOID && pi == SQLITE_ELLIPSE_CIRCLE_AVOID ) {
				return SQLITE_BOX_CONT_ELLIPSE;
			} else {
				return SQLITE_BOX_ELLIPSE_OVER;
			}
	...
	if ( pw == SQLITE_ELLIPSE_LINE_AVOID &&
			pe == SQLITE_ELLIPSE_LINE_AVOID ) {
		...
		if ( pw == SQLITE_ELLIPSE_CONT_LINE && pe == SQLITE_ELLIPSE_CONT_LINE &&
...
/*!
	...
	\return relationship as a \link SQLITE_BOX_REL long value \endlink (\ref  SQLITE_BOX_REL )
*/
static long sbox_box_pos ( const SBox * b1 , const SBox * b2 , bool recheck) {
	...
	if ( spoint_eq( &b2->sw, &b2->ne) ) {
		...
		if ( scp ) {
			return SQLITE_BOX_CONT;
		} else {
			return SQLITE_BOX_AVOID;
		}
	}
	...
	if ( scp && pe == SQLITE_BOX_CONT_LINE && pw == SQLITE_BOX_CONT_LINE ) {
		return SQLITE_BOX_CONT;
	}
	if ( pe > SQLITE_BOX_LINE_AVOID || pw > SQLITE_BOX_LINE_AVOID ) {
		return SQLITE_BOX_OVER;
	}

	if ( !recheck ) {
		pe = sbox_box_pos ( b2, b1, true );
		if ( pe == SQLITE_BOX_CONT ) {
			return SQLITE_BOX_OVER;
		}
	}
	return SQLITE_BOX_AVOID;
...
bool spherebox_cont_circle(SBox * b, SCircle * c) {
	...
	return ( pos == SQLITE_BOX_CONT_CIRCLE || pos == SQLITE_BOX_CIRCLE_EQUAL );
}
...
bool spherecircle_cont_box(SCircle * c, SBox * b) {
	int pos = sbox_circle_pos ( c, b );
	return ( pos == SQLITE_CIRCLE_CONT_BOX || pos == SQLITE_BOX_CIRCLE_EQUAL );
}
...
bool spherebox_overlap_circle(SBox * b, SCircle * c) {
	return ( sbox_circle_pos ( c , b ) > SQLITE_BOX_CIRCLE_AVOID );
}
...
bool spherebox_cont_line(SBox * b, SLine * l) {
	return ( sbox_line_pos ( l , b ) == SQLITE_BOX_CONT_LINE );
}
...
bool spherebox_overlap_line(SBox * b, SLine * l) {
	return ( sbox_line_pos ( l , b ) > SQLITE_BOX_LINE_AVOID );
}
...
bool spherebox_cont_path(SBox * b, SPath * p) {
	return ( sbox_path_pos ( p , b ) == SQLITE_BOX_CONT_PATH );
}
...
bool spherebox_overlap_path(SBox * b, SPath * p) {
	return ( sbox_path_pos ( p , b ) > SQLITE_BOX_PATH_AVOID );
}
...
bool spherebox_cont_poly(SBox * b, SPoly * p) {
	return ( sbox_poly_pos ( p , b ) == SQLITE_BOX_CONT_POLY );
}
...
bool spherepoly_cont_box(SPoly * p, SBox * b) {
	return ( sbox_poly_pos ( p , b ) == SQLITE_POLY_CONT_BOX );
}
...
bool spherebox_overlap_poly(SBox * b, SPoly * p) {
	return ( sbox_poly_pos ( p , b ) > SQLITE_BOX_POLY_AVOID );
}
...
bool spherebox_cont_ellipse(SBox * b, SEllipse * e) {
	return ( sbox_ellipse_pos ( e , b ) == SQLITE_BOX_CONT_ELLIPSE );
}
...
bool sphereellipse_cont_box(SEllipse * e, SBox * b) {
	return ( sbox_ellipse_pos ( e , b ) == SQLITE_ELLIPSE_CONT_BOX );
}
...
bool spherebox_overlap_ellipse(SBox * b, SEllipse * e) {
	return ( sbox_ellipse_pos ( e , b ) > SQLITE_BOX_ELLIPSE_AVOID );
}
...
bool spherebox_cont_box(SBox * box1, SBox * box2) {
	return  ( sbox_box_pos ( box1, box2, false ) ==  SQLITE_BOX_CONT );
}
...
bool spherebox_overlap_box(SBox * box1, SBox * box2) {
	return  ( sbox_box_pos ( box1, box2, false ) >  SQLITE_BOX_AVOID );
}
